...
**Email across the lifecycle.** Owners are emailed when a match is found and when their claim is approved (with the pickup code). Finders are emailed when the owner is verified and when the item is collected. Emails are sent from a **background thread**, so a slow mail server never delays a report. Any mail error is logged and ignored, so it can't break a claim. Without mail settings, emails are printed to the console. Email goes through Brevo's HTTPS API when `BREVO_API_KEY` is set, because many hosts (including Railway's lower plans) block outgoing SMTP ports. Otherwise it uses any SMTP server via Python's `smtplib`.
